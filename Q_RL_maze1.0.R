...
actions <- c('up', 'down', 'left', 'right')  # movement

# create a list
q_table <- data.frame(matrix(ncol = 5, nrow = 0))   # create empty Q-Table
...
## Drawing and movement in maze. R is clearly not intendet to do animations!
##############################################################################################################
maze0<-function(){
...
maze1<-function(x,y){
  symbols(x,y,circles = 0.4,inches = FALSE, fg='white',bg='red',add = T)
  Sys.sleep(0.2)
}

maze2<-function(x,y){
  # clears old symbol after terminal state
  symbols(x,y,circles = 0.4,inches = FALSE, fg='white',bg='white',add = T)
}

maze3<-function(){
  Sys.sleep(0.1) #cleares after terminal state
  symbols(3,2,squares = 0.8,inches = FALSE, fg='yellow',bg='yellow',add = T)
  symbols(2,2,squares = 0.8,inches = FALSE, fg='black',bg='black',add = T)
  symbols(3,3,squares = 0.8,inches = FALSE, fg='black',bg='black',add = T)
}

#################################################################################################################

check_state_exists <- function(self,state){
  #check if this state is already in q_table and enters zeros if not
  #print(state)
...
update <- function(){
  ...
  for(episode in 1:50){
    ...
    while(done==F){
      ...
      self<-check_state_exists(self, ob_r_d$s_) #check if future state exists
